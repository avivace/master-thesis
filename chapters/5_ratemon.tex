...
The \mcode{runstandalone} method \cite{RestructuretheplotTriggerRatescripttobeusedasamodule7MergeRequestsCMSTSGFOGratemonGitLab-2020-10-07} exposes the trigger rates in a transparent way. Users can now import the RateMon tools, configure the database connection and get Trigger Rates from any run or fill in any of the available units (and corrections, see \ref{expoptions}) as time series in Python objects.
...
\end{textcode}
\caption{SELinux troubleshooting and configuration}
\end{listing}

Listing \ref{nginx} describes how we are exposing the API server using the reverse proxy feature and how the proxy is configure to retain data for 200 days for a maximum total size of 500G (as the volume we configured).
...
\end{nginxcode}
\caption{NGINX configuration for reverse proxying and caching\label{nginx}}
\end{listing}
...
Configuring a single configuration variable to plug the endpoint exposing an instance of the RateMon API, it is possible to deploy the application just by installing the NodeJS dependencies and the running a single \mcode{npm run start} command.
