...
A \textit{prescale} is a way of scaling back the rate of L1 and HLT triggers. For example, if a certain trigger has a prescale of 5, then the data corresponding to the event the trigger fired on is only recorded once out of every five times that the trigger fired. So if the prescale is 1, the data will be recorded every time the trigger fires. A prescale of 0 means that no data will be recorded for the trigger in question: the trigger is essentially just turned off. 
Correcting for deadtime in Rate VS PU plots enables consistent comparisons of rates for different runs/fills and across (different) pre scale columns. 
...
RPM packages can be obtained on public CERNBox or Dropbox folders or automatically installed in Puppet-managed machines, depending on the environemnt and system administration policy.
...
The software requires a connection to the CMS OMDS database. Previously, these credentials had to be filled directly inside a python file imported by the application entry point.
