...
Code snippets demonstrating the execution of commands in a shell are noted with the \texttt{\$} character. Preceding the \texttt{\$} character you sometimes notice a string specifying the hostname of the machine (or the general infrastructure) in which that commands must run to success (and make sense). E.g. \texttt{lxplus \$ command} shows the command execution on one of the machine from LXPLUS, the CERN service offering access to machines running Linux CERN CentOS 7.

If there's no specification before \texttt{\$}, it means the machine or the environment is not relevant.
